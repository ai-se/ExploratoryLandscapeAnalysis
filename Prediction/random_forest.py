...
def getn_way(filename):
    ...
    dataset_column = [c for c in content.columns if "$$" in c]
    datasets = [str(content[dataset_column].iloc[i].tolist()[-1]) for i in xrange(len(content))]
    datasets = list(set([d.split("_")[0] for d in datasets]))
