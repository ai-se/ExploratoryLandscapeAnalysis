...
files = ["../FeatureModels/" + f for f in os.listdir("../FeatureModels") if ".csv" in f]
for file in files:
    df = pd.read_csv(file)
    nof = len(df.columns)
    print_name = file.split("/")[-1].split(".")[0]
    print "get_landscape_features(\"~/GIT/ExploratoryLandscapeAnalysis/FeatureModels/" + print_name + ".csv\"," + str(nof-1) + ',' + str(nof)  + ", \"~/GIT/ExploratoryLandscapeAnalysis/Result_FeatureModels/"+ print_name +".txt\")"
